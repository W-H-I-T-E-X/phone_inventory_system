...
clear
echo "╔═══════════════════════════════════════╗" | lolcat
echo "║    KM TRACKER → AMOUNT CALCULATOR     ║" | lolcat
echo "╚═══════════════════════════════════════╝" | lolcat
echo ""

# Get day range

read -p $'\e[1;36mEnter START DAY (e.g., 01): \e[0m' start_day
read -p $'\e[1;36mEnter END DAY   (e.g., 10): \e[0m' end_day
start_day=$(printf "%02d" $((10#$start_day)))
end_day=$(printf "%02d" $((10#$end_day)))

echo ""

echo "You are entering data for:" | lolcat
echo -e "\e[1;33m▶ ${year}-${month}-${start_day} to ${year}-${month}-${end_day}\e[0m"
echo ""

# Table headers
header_fmt="%-12s  │ %-8s │ %-8s │ %-10s │ %-10s\n"
divider="──────────────┼──────────┼──────────┼────────────┼────────────"

echo "$divider"
printf "$header_fmt"  " DATE" "START" "END" "KILOMETER" "AMOUNT"
echo "$divider"
...
for (( day=10#$start_day; day<=10#$end_day; day++ )); do
  ...
  echo " "
  read -p $'\e[1;35m['"$date"$'] Enter START km: \e[0m' start_km
  read -p $'\e[1;34m['"$date"$'] Enter END km:   \e[0m' end_km
  echo " "
  km=$((end_km - start_km))
  amt=$((km * 3))
  ...
  printf "$header_fmt" "$date" "$start_km" "$end_km" "$km" "₹$amt"
  rows+=("$date $start_km $end_km $km ₹$amt")
done
...
for row in "${rows[@]}"; do
  echo "$row" | awk '{printf "%-12s  │ %-8s │ %-8s │ %-10s │ %-10s\n", $1, $2, $3, $4, $5}'
done

echo "$divider"
printf "\e[1;32m%-33s   │ %-10s │ %-10s\e[0m\n" "TOTAL" "$total_km km" "₹$total_amt"
echo "$divider" | lolcat
}
...
n=$(printf "%s\n" "${phone_list[@]}" | fzf --prompt="Select Phone Name: ")
if [[ -z "$n" ]]; then
  echo -n "NAME => "
  read n
fi
...
s=$(printf "%s\n" "${storage[@]}" | fzf --prompt="Select Phone Storage: ")
...
#read i
# Dependencies: tesseract, lolcat, maim (X11) or grim+slurp (Wayland)
# Make sure these are installed.
img="/tmp/imei_capture.png"
cleaned="/tmp/cleaned_imei.png"

# Countdown before capture
for j in {3..1}; do
  echo -ne "Capturing in $j seconds...\r" | lolcat
  sleep 1
done

# Screenshot (Wayland or X11)
...
# Safely preprocess image (no white-out)
magick "$img" \
  -resize 300% \
  -colorspace Gray \
  -contrast-stretch 0 \
...
# Run OCR with relaxed matching (handles space-separated IMEI)
i=$(tesseract "$cleaned" - -l eng --dpi 300 --psm 7 \
     | grep -oE '([0-9]{2}[ ]?[0-9]{6}[ ]?[0-9]{6}[ ]?[0-9])' \
     | tr -d ' ' | head -n1)

echo ""
if [[ -n "$i" ]]; then
  echo -e "IMEI => $i" | lolcat
else
  echo -ne "IMEI => " | lolcat
  read i
fi
...
c_name=(
	...
	"FAHEEM"
	"FAHEEM BHAI"
	"FAHEEM BHAI FAISAL ROAMIO"
	"FAHEEM BHAI FRIEND [JAWAD BAHI]"
	"FAHEEM BHAI OFFICE"
	"FAIZAN"
	"FARHAN"
	"FARHAN KHAN"
	"FARHAT BHAI"
	"FARHAT BHAI PERSONAL"
	"FURKAN"
	"GAGAN"
	"GARUV"
	"GHUNJAN"
...
c=$(printf "%s\n" "${c_name[@]}" | fzf --prompt="Select Customer Name: ")
if [[ -z "$c" ]]; then
  echo -n "C_NAME => "
  read c
fi
...
# Use fzf for table selection
table=$(echo "$tables" | fzf --prompt="CHOOSE A TABLE: " --height=10 --border)
...
echo " "
if [[ -n "$has_date" ]]; then
  echo -e "📆 The table '$table' has a DATE column. Do you want to filter by month? (y/n) => \c" | lolcat
  read -r month_choice
else
  echo -e "⚠️  The table '$table' does NOT have a DATE column. Showing full table." | lolcat
  month_choice="n"
fi

# If user wants month filtering
if [[ "$month_choice" == "y" || "$month_choice" == "Y" ]]; then
  ...
  echo " "
  echo -e "📋 DISPLAYING FULL TABLE: '$table'" | lolcat
  echo " "
  sudo mariadb -D "$DB" -e "SELECT * FROM ${table};"
fi
...
case $b in
  1)
    ...
    ;;
  2)
    start # assume start is your main menu function
    ;;
  *)
    echo "Invalid option."
    ;;
...
n=$(printf "%s\n" stock p_table s_table p_acc s_acc repair exp parts charges sal pen_h pen_rec acc_stock | fzf --prompt="Table: ")
[[ -z "$n" ]] && { echo "No table selected. Exiting."; exit 1; }

columns=$(sudo mariadb -D cashphones -sN -e "SHOW COLUMNS FROM ${n};" | awk '{print $1}')
echo " "

# Ask how many identifier columns
echo -e "HOW MANY IDENTIFIER COLUMNS DO YOU WANT? (1 or 2)" | lolcat
read id_count

if [[ "$id_count" == "2" ]]; then
  echo -e "CHOOSE IDENTIFIER COLUMN 1:" | lolcat
  id_column1=$(echo "$columns" | fzf --prompt="Column 1: ")
  [[ -z "$id_column1" ]] && { echo "No column selected. Exiting."; exit 1; }

  echo -e "CHOOSE IDENTIFIER COLUMN 2:" | lolcat
  id_column2=$(echo "$columns" | grep -v "^${id_column1}$" | fzf --prompt="Column 2: ")
  [[ -z "$id_column2" ]] && { echo "No second column selected. Exiting."; exit 1; }

  echo -e -n "VALUE OF '$id_column1' => " | lolcat
  read id_value1
  echo -e -n "VALUE OF '$id_column2' => " | lolcat
  read id_value2

else
  echo -e "CHOOSE IDENTIFIER COLUMN:" | lolcat
  id_column1=$(echo "$columns" | fzf --prompt="Column: ")
  [[ -z "$id_column1" ]] && { echo "No column selected. Exiting."; exit 1; }
...
# Function to fetch the matching row
fetch_row() {
  if [[ "$id_count" == "2" ]]; then
    sudo mariadb -D cashphones -e "SELECT * FROM ${n} WHERE ${id_column1} = '${id_value1}' AND ${id_column2} = '${id_value2}';"
  else
    sudo mariadb -D cashphones -e "SELECT * FROM ${n} WHERE ${id_column1} = '${id_value1}';"
  fi
}
...
while true; do
  ...
  c=$(echo "$columns" | fzf --prompt="Update column: ")
  [[ -z "$c" ]] && { echo "No column selected. Exiting."; break; }

  echo -e -n "NEW VALUE FOR '$c' => " | lolcat
  read v

  # Build WHERE clause based on number of identifiers
  if [[ "$id_count" == "2" ]]; then
    [[ "$c" == "$id_column1" ]] && id_value1="$v"
    [[ "$c" == "$id_column2" ]] && id_value2="$v"
    sudo mariadb -D cashphones -e "UPDATE ${n} SET ${c}='${v}' WHERE ${id_column1} = '${id_value1}' AND ${id_column2} = '${id_value2}';"
  else
    [[ "$c" == "$id_column1" ]] && id_value1="$v"
    sudo mariadb -D cashphones -e "UPDATE ${n} SET ${c}='${v}' WHERE ${id_column1} = '${id_value1}';"
  fi
  ...
  echo " "
  echo -e "1) REPEAT (Update more columns in same row)" | lolcat
  echo -e "2) BACK" | lolcat
  echo " "
  echo -e -n "==> "
  read b
...
# Ask how many identifier columns
echo " "
echo -e -n "HOW MANY IDENTIFIER COLUMNS DO YOU WANT? (1 or 2) => " | lolcat
read -r id_count
...
echo -e -n "ENTER VALUE FOR '$col1' (LIKE) => " | lolcat
read -r val1

# --------- Optional Second column match ----------
if [[ "$id_count" == "2" ]]; then
  echo " "
  ...
  echo -e -n "ENTER VALUE FOR '$col2' (LIKE) => " | lolcat
  read -r val2
fi

# --------- Build WHERE and SHOW queries ----------
echo " "
if [[ "$id_count" == "2" ]]; then
  where_clause="${col1} LIKE '%${val1}%' AND ${col2} LIKE '%${val2}%'"
else
  where_clause="${col1} LIKE '%${val1}%'"
fi
...
# --------- Confirm and delete ----------
echo " "
echo -e -n "ARE YOU SURE YOU WANT TO DELETE THESE ROWS? (yes/no): " | lolcat
read -r confirm

if [[ "$confirm" == "yes" ]]; then
  del_query="DELETE FROM ${table} WHERE ${where_clause};"
  sudo mariadb -D "$DB" -e "$del_query"
  echo -e "\nROWS DELETED WHERE $where_clause" | lolcat
...
case $b in
  1)
    ...
    ;;
  2)
    start # Call start function or menu
    ;;
  *)
    echo "Invalid option."
    ;;
...
ORDER BY
  CASE
    WHEN IMEI = '============' THEN 2
    WHEN IMEI = '$today' THEN 3
    ELSE 1
  END,
  -- First by brand (e.g., IPHONE, SAMSUNG, etc.)
  SUBSTRING_INDEX(NAME, ' ', 1),
  -- Then by model (e.g., 11, 12 PRO, XR...)
  TRIM(SUBSTRING(NAME, LOCATE(' ', NAME)));
"
echo " "
}
...
case $n in
	1)
		echo -e -n "MONTH => "|lolcat
		read m
		sudo mariadb -D cashphones -e "select ROW_NUMBER() OVER (ORDER BY DATE) AS S_NO,DATE,ORDER_ID,NAME,TOTAL,QNTY from s_table WHERE DATE LIKE '2025-${m}-%' union select '=======','===========','======','============================','========','=====' from s_table union all SELECT '---->','SALES','TABLE','<---------> TOTAL <--------->',SUM(TOTAL),SUM(QNTY) FROM s_table WHERE DATE LIKE '2025-${m}-%';"
		;;
	2)
		echo -e -n "MONTH => "|lolcat
		read m
		sudo mariadb -D cashphones -e "select ROW_NUMBER() OVER (ORDER BY DATE) AS S_NO,DATE,ORDER_ID,NAME,TOTAL,QNTY from p_table WHERE DATE LIKE '2025-${m}-%' union select '=======','===========','=======','===================================','========','====' from p_table union all SELECT '---->','PURCHASE','TABLE','<------------> TOTAL <------------>',SUM(TOTAL),SUM(QNTY) FROM p_table WHERE DATE LIKE '2025-${m}-%';"
		;;		
esac
echo " "
...
count(){
	...
	echo "-------"
	echo
	sudo mariadb -D cashphones -e "SELECT ORDER_ID,NAME,STORAGE,C_NAME,IMEI,DATE,CASH FROM p_table WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "P_ACCESSORIES"
	echo "-------------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM p_acc WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "EXPANSES"
	echo "--------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM exp WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "FUND SUBTRACT"
	echo "-------------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM fund_sub WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "PENDING"
	echo "-------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,ORDER_ID,NAME,CASH FROM pen_h WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "PURCHASE-PAYMENT-SEND"
	echo "---------------------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,ORDER_ID,NAME,CASH FROM p_send WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "SALARY"
	echo "------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM sal WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "PARTS"
	echo "-----"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM parts WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "S_PHONE"
	echo "-------"
	echo
	sudo mariadb -D cashphones -e "SELECT ORDER_ID,NAME,STORAGE,C_NAME,IMEI,DATE,CASH FROM s_table WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "REPAIR"
	echo "------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM repair WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "S_ACCESSORIES"
	echo "-------------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM s_acc WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "FUNDS ADD"
	echo "---------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,NAME,CASH FROM fund_add WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "PENDING REC"
	echo "-----------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,ORDER_ID,NAME,CASH FROM pen_rec WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
	
	echo "PURCHASE-PAYMENT-DELAY"
	echo "----------------------"
	echo
	sudo mariadb -D cashphones -e "SELECT DATE,ORDER_ID,NAME,CASH FROM p_delay WHERE DATE BETWEEN '$range_start' AND '$range_end' AND CASH <> 0 ORDER BY DATE;"
	echo
